    ...
    <div className="space-y-6 animate-in">
      <div className="flex items-center justify-between">
        ...
        <button type="button" onClick={() => setCreate(c => ({...c, show: !c.show}))}
          ...
          {create.show ? "Cancel" : "+ New Task"}
        </button>
      </div>

      {create.show && (
        <div className="bg-card border border-border rounded-xl p-5 space-y-3">
          <input aria-label="Task title" className="w-full bg-surface border border-border rounded-lg px-4 py-2.5 text-sm outline-none focus:border-accent"
            placeholder="Task title" value={create.title} onChange={(e) => setCreate(c => ({...c, title: e.target.value}))}
            onKeyDown={(e) => e.key === "Enter" && createTask()} />
          <input aria-label="Task description" className="w-full bg-surface border border-border rounded-lg px-4 py-2.5 text-sm outline-none focus:border-accent"
            placeholder="Description (optional)" value={create.desc} onChange={(e) => setCreate(c => ({...c, desc: e.target.value}))} />
          <div className="flex gap-2 items-center">
            ...
            <button type="button" onClick={createTask} disabled={!create.title.trim()}
